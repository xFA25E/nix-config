...
The most tricky part is in ~lisp-with-parens~.  It surrounds a /tempo/ templates
with parens, only if it detects that there was no open paren at the beginning.
It is used to for lisp abbrevs.  A template needs to expand correctly after
~word SPC~ and after ~(word SPC~.
