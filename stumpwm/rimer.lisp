...
(defun parse-rimer-report (text)
  (if-let ((jsown-package (find-package "JSOWN")))
    (let ((parse (symbol-function (find-symbol "PARSE" jsown-package)))
      ...
      (mapcar
       (lambda (obj)
         (list (funcall val obj "name")
               (funcall val (funcall val obj "elapsed") "secs")
               (funcall val (funcall val obj "duration") "secs")
               (let ((state (funcall val obj "state")))
                 (cond ((string-equal "running" state) :running)
                       ((string-equal "paused" state) :paused)
                       ((string-equal "halted" state) :halted)))))
       (funcall parse text)))
    (progn (message "No jsown!") nil)))
